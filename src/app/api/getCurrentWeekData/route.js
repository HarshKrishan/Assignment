...
export async function GET(req) {
  ...
  //     `SELECT UID, Name, Score, Country, TimeStamp, RANK() OVER (ORDER BY Score DESC) AS UserRank FROM
	// user WHERE YEARWEEK(TimeStamp) = YEARWEEK(CURDATE()) ORDER BY UserRank LIMIT 200;`)
  //   .then(([data, fields]) => {
  //     // console.log(data);
  //     return data;
  //   })
  ...
  const result =
    await sql`SELECT UID, Name, Score, Country, TimeStamp, RANK() OVER (ORDER BY Score DESC) AS UserRank FROM user WHERE YEARWEEK(TimeStamp) = YEARWEEK(CURDATE()) ORDER BY UserRank LIMIT 200;`
    .then((data) => {
      // console.log(data);
      return data;
    }
